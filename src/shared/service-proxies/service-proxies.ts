...
export class CountryControllerServiceProxy {
    ...
    changeStatus(cid: number, status: string): Observable<Country> {
        let url_ = this.baseUrl + "/country/countryactivation1?";
        if (cid === undefined || cid === null)
            throw new Error("The parameter 'cid' must be defined and cannot be null.");
        else
    ...
    protected processChangeStatus(response: HttpResponseBase): Observable<Country> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
